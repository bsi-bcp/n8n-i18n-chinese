...
8. **发布**：`git tag release/<ver> && git push origin release/<ver>` → `gh release create release/<ver> editor-ui-<ver>.tar.gz`。🔴 **必须加 `-R bsi-bcp/n8n-i18n-chinese`**——本地配了 `upstream` remote 时 gh 会推断成已归档的上游仓库并拒绝（"tag exists locally but has not been pushed to other-blowsnow/..."）。若 CI 已建同名 tag/Release，改用 `gh release upload release/<ver> editor-ui-<ver>.tar.gz --clobber -R bsi-bcp/n8n-i18n-chinese` 追加/替换附件
