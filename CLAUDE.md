...
6. **打包**：`cp NOTICE-n8n-localization.md THIRD_PARTY_LICENSES.md packages/frontend/editor-ui/dist/ && tar -czf "n8n-editor-ui@<ver>.tar.gz" -C packages/frontend/editor-ui dist`（SUL 随附条款随包分发，评审 D-E5-1/D-E5-8；第三方依赖许可清单 2026-09-19 起随包分发——取自官方发行物、覆盖 n8n 全体依赖，对 dist 前端子集属从宽声明，镜像侧 `/usr/local/lib/node_modules/n8n/THIRD_PARTY_LICENSES.md` 为精确口径；上游大版本升级时刷新该文件；macOS tar 的 LIBARCHIVE.xattr 警告在 Linux 解包无害）
