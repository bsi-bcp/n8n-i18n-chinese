...
7. **YTJ1 迁移按 minor SOP**：冷备 → db 迁移窗口 → 升级预告发在管客户（乙方排期制下预告是交付动作不是可选项）。✅ **已完成**：minor 跨级（2.39.8→**2.40.5**，含 DB 迁移）实际发生于 **2026-09-22**；**2026-09-25 再升 patch 级 2.40.5→2.40.7**（冷备：YTJ1 配置四件套 + YTJ2 `pg_dump -Fc` 21.4MB/147 表 + 本机副本 + DinD 沙箱镜像 306MB）
8. **YTJ1 抽查**：AI Agent/Chat Model 面板中文（langchain 注入首验）+ 新拖节点画布默认名 + 对照本版新增译串。✅ **2026-09-25 复验全绿**：`5680/healthz` 200 + `readiness` ok + 公网 200 + `n8n --version`=2.40.7 + 混版告警 0 + 社区节点 0 报错 + runners 注册 + sandbox-api ok + **138 流/3 active 与升级前一致** + 品牌 title `BCP-HUB 软商`
