...
// 替换进 n8n-nodes-base dist 的 JS 文件——参数面板串硬编码于节点类文件、
// 经后端 types payload 直出、前端渲染不过 i18n 查表（审计实证），
// 故在构建期对 dist 做定向替换（M0 POC 在 YTJ1 实机验证过该注入层）。
//
// 用法: node inject-params.cjs <nodes-base dist 目录> <params-zh-map.json>
//
// 安全设计（立项风险表 D-P1/B）：
...
// - 每文件替换后 node --check 语法校验，坏即回滚该文件并计数告警
const fs = require('fs');
...
const hitEn = new Set(); // 仅落盘成功后计入（利用率口径=实际生效）
