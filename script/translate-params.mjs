...
items = items.slice(0, topN).map(([en, meta]) => ({ en, count: meta.count, nodes: meta.nodes }));
...
4. 输出：一个 JSON 对象，键为原文（逐字），值为中文译文；必须覆盖输入的每一条，不得增删键
...
async function translateBatch(batch, attempt = 1) {
  const payload = {
    ...
    messages: [
      ...
      { role: 'user', content: JSON.stringify(batch.map((b, i) => ({ i, en: b.en, ctx: (b.nodes || [])[0] || '' }))) },
    ],
  };
  ...
  for (const b of batch) {
    ...
    if (isBad(b.en, zh)) { bad++; continue; }
    out[b.en] = zh;
  }
  if (bad) console.log(`  ⚠️ ${bad} 条被安全校验跳过`);
  return out;
}
...
(async () => {
  ...
  for (let i = 0; i < items.length; i += BATCH) {
    ...
    if (done % 300 === 0 || done >= items.length) console.log(`进度 ${done}/${items.length}, 已译 ${Object.keys(result).length}`);
  }
