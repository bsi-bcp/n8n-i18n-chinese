import { $ as openBlock, A as createTextVNode, C as createBaseVNode, Cn as toDisplayString, E as createElementBlock, F as guardReactiveProps, Gt as unref, Ht as toRefs, It as ref, N as defineComponent, P as getCurrentInstance, S as computed, T as createCommentVNode, U as mergeProps, W as nextTick, X as onMounted, Z as onUnmounted, _ as Fragment, _t as watchEffect, bn as normalizeStyle, bt as withCtx, c as useCssModule, gt as watch, h as withModifiers, it as renderSlot, j as createVNode, lt as useAttrs, pt as useTemplateRef, q as onBeforeUnmount, rt as renderList, st as resolveDynamicComponent, vn as normalizeClass, w as createBlock, y as Teleport, yn as normalizeProps } from "./vue.runtime.esm-bundler-DYHsQBZB.js";
import { t as useI18n } from "./useI18n-D97R0NGZ.js";
import { t as _plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-D-F0WtqU.js";
import { t as Icon_default } from "./Icon-CN0UVd9e.js";
import { L as reactivePick } from "./dist-CaaDNBsJ.js";
import { n as isEqual, t as useFormControl } from "./useFormControl-CO0Elh2N.js";
import { t as clamp } from "./clamp-CGW4Q_X6.js";
import { A as createContext, a as useId, f as useResizeObserver, l as unrefElement, o as useForwardProps, p as useVModel, r as Presence_default, s as useForwardExpose, t as Primitive } from "./Primitive-BLGZcC0m.js";
import { n as getActiveElement, t as useDirection } from "./useDirection-Dyw85FIT.js";
...
import { t as isNullish } from "./nullish-NCVJyheq.js";
import { n as useHideOthers, r as useBodyScrollLock, t as FocusScope_default } from "./FocusScope-CIk0dDnv.js";
import { t as useFocusGuards } from "./useFocusGuards-BSj0kf8L.js";
import { t as useTypeahead } from "./useTypeahead-C0tIP6oc.js";
import { s as focusFirst } from "./utils-5g_9By3F.js";
import { t as useCollection } from "./Collection-BHvZHttL.js";
import { t as VisuallyHidden_default } from "./VisuallyHidden-D1XyXEcX.js";
import { t as useNonce } from "./useNonce-B7SY2PB2.js";
	...
	setup(__props) {
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(SelectItem_default$1), {
				...
				value: props.value,
				"text-value": typeaheadText.value,
...
var SelectItem_vue_vue_type_style_index_0_lang_module_default = {
	itemLeading: "_itemLeading_57w53_2",
	itemText: "_itemText_57w53_7",
	itemIndicator: "_itemIndicator_57w53_16",
	itemTrailing: "_itemTrailing_57w53_17"
};
...
var Select_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const forwardedRootProps = useForwardProps(reactivePick(props, "open", "defaultOpen", "disabled", "required", "multiple"));
		function rootBind() {
			return {
		...
		__expose({ triggerRef: useTemplateRef("trigger") });
		const sizes = {
			mini: $style.mini,
			small: $style.small,
			medium: $style.medium,
		...
		function warnInvalidItem(message, item) {}
		function normaliseOption(item) {
			if (item.value === "") {
				warnInvalidItem("Skipping item: \"value\" is missing or empty. Every selectable item needs a non-empty value.", item);
				return;
			}
			if (!item.label) {
				warnInvalidItem("Skipping item: \"label\" is missing or empty. Every selectable item needs a label.", item);
				return;
			}
			return {
				...item,
			...
			for (const item of props.items) {
				if (isGroupItem(item)) {
					...
					for (const child of item.items) {
						...
						if (normalised) groupItems.push(normalised);
					}
					const label = item.label || void 0;
					if (item.label !== void 0 && !item.label) warnInvalidItem("Skipping group label: \"label\" is empty.", item);
					result.push({
						type: "group",
						group: item,
						label,
				...
				if (normalised) pendingOptions.push(normalised);
			}
			flushPendingOptions();
			return result;
		});
		const optionItems = computed(() => sections.value.flatMap((section) => section.type === "group" ? section.items : []));
		const hasSelectableItems = computed(() => sections.value.some((section) => section.type === "group" && section.items.length > 0));
		...
		function selectedIconUi() {
			return {
				class: $style.selectedIconGlyph,
				strokeWidth: iconStrokeWidth()
			};
		}
		function getSelectedItem(value) {
			if (props.multiple) return;
			if (value === void 0 || value === null || Array.isArray(value)) return;
			return optionItems.value.find((item) => item.value === value);
		}
		function isModelValue(value) {
			if (props.multiple) return Array.isArray(value) && value.every(isSelectValue);
			return isSelectValue(value);
		}
		function setSelectedValue(value) {
			if (!isModelControlled) internalValue.value = value;
			emit("update:modelValue", value);
		}
		function onClear() {
			if (props.multiple) {
				const empty = [];
				if (isModelValue(empty)) setSelectedValue(empty);
			} else setSelectedValue(void 0);
			emit("clear");
		}
		function handleModelValueUpdate(value) {
			if (!isModelValue(value)) return;
			setSelectedValue(value);
		}
		function slotModelValue(value) {
			if (value === null || value === void 0) return;
			return isModelValue(value) ? value : void 0;
		}
		function resolveDisplayValue(value) {
			...
			if (Array.isArray(value)) {
				const labels = value.map((entry) => {
					return items.find((item) => item.value === entry)?.label ?? stringifyPrimitive(entry);
		...
		return (_ctx, _cache) => {
			return openBlock(), createBlock(unref(SelectRoot_default), mergeProps(rootBind(), {
				"model-value": rootModelValue.value,
				"onUpdate:modelValue": handleModelValueUpdate,
				"onUpdate:open": _cache[2] || (_cache[2] = ($event) => emit("update:open", $event))
			}), {
				default: withCtx(({ open: isMenuOpen, modelValue: selectedValue }) => [createVNode(unref(SelectTrigger_default), mergeProps({
					id: __props.id,
					ref: "trigger",
					as: "div",
					...
					default: withCtx(() => [
						(openBlock(true), createElementBlock(Fragment, null, renderList([getSelectedItem(selectedValue)], (selectedItem) => {
							return openBlock(), createElementBlock(Fragment, { key: selectedItem?.value ?? "none" }, [!__props.multiple && selectedItem && (selectedItem.icon || _ctx.$slots["item-leading"]) ? (openBlock(), createElementBlock("span", {
								key: 0,
								class: normalizeClass(unref($style).selectedIcon)
							}, [renderSlot(_ctx.$slots, "item-leading", {
								item: selectedItem,
								ui: selectedIconUi()
							}, () => [selectedItem.icon ? (openBlock(), createBlock(Icon_default, mergeProps({
								key: 0,
								icon: selectedItem.icon,
								color: "text-base"
							}, { ref_for: true }, selectedIconUi()), null, 16, ["icon"])) : createCommentVNode("", true)])], 2)) : __props.icon ? (openBlock(), createElementBlock("span", {
								key: 1,
								class: normalizeClass(unref($style).selectedIcon)
							}, [createVNode(Icon_default, mergeProps({
								icon: __props.icon,
								color: "text-base"
							}, { ref_for: true }, selectedIconUi()), null, 16, ["icon"])], 2)) : createCommentVNode("", true)], 64);
						}), 128)),
						createVNode(unref(SelectValue_default), {
							...
							default: withCtx(() => [renderSlot(_ctx.$slots, "default", {
								modelValue: slotModelValue(selectedValue),
								open: isMenuOpen
							}, () => [createTextVNode(toDisplayString(resolveDisplayValue(selectedValue) ?? resolvedPlaceholder()), 1)])]),
							_: 2
						}, 1032, ["placeholder", "class"]),
						showClearButton(selectedValue) ? (openBlock(), createElementBlock("button", {
							key: 0,
							type: "button",
							"data-test-id": "select-clear",
							class: normalizeClass(unref($style).clearButton),
							"aria-label": unref(t)("nds.select.clear"),
							onMousedown: _cache[0] || (_cache[0] = withModifiers(() => {}, ["prevent"])),
							onPointerdown: _cache[1] || (_cache[1] = withModifiers(() => {}, ["stop", "prevent"])),
					...
					default: withCtx(() => [createVNode(unref(SelectContent_default), {
						...
						default: withCtx(() => [
							...
							createBaseVNode("div", { class: normalizeClass(unref($style).viewportRegion) }, [
								hasSelectableItems.value ? (openBlock(), createBlock(unref(SelectScrollUpButton_default), {
									key: 0,
									class: normalizeClass([unref($style).selectScrollButton, unref($style).selectScrollButtonUp]),
								...
								createVNode(unref(SelectViewport_default), { class: normalizeClass(unref($style).selectViewport) }, {
									default: withCtx(() => [!hasSelectableItems.value ? (openBlock(), createElementBlock("div", {
										key: 0,
										class: normalizeClass(unref($style).empty),
										"data-test-id": "select-empty"
									}, [renderSlot(_ctx.$slots, "empty", {}, () => [createTextVNode(toDisplayString(unref(t)("nds.select.noResults")), 1)])], 2)) : (openBlock(true), createElementBlock(Fragment, { key: 1 }, renderList(sections.value, (section, sectionIndex) => {
										return openBlock(), createElementBlock(Fragment, { key: `section-${sectionIndex}` }, [section.type === "separator" ? (openBlock(), createBlock(unref(SelectSeparator_default), {
											key: 0,
											class: normalizeClass(unref($style).selectSeparator),
											"aria-hidden": "true"
										}, null, 8, ["class"])) : (openBlock(), createBlock(unref(SelectGroup_default), { key: 1 }, {
											default: withCtx(() => [section.label && section.group ? (openBlock(), createBlock(unref(SelectLabel_default), {
												key: 0,
												class: normalizeClass(unref($style).selectLabel)
											}, {
												default: withCtx(() => [renderSlot(_ctx.$slots, "label", { item: section.group }, () => [createTextVNode(toDisplayString(section.label), 1)])]),
												_: 2
											}, 1032, ["class"])) : createCommentVNode("", true), (openBlock(true), createElementBlock(Fragment, null, renderList(section.items, (item) => {
												return renderSlot(_ctx.$slots, "item", {
													key: `section-${sectionIndex}-item-${String(item.value)}`,
													item
												}, () => [createVNode(SelectItem_default, mergeProps({ ref_for: true }, item, {
													class: unref($style).selectItem,
								...
								hasSelectableItems.value ? (openBlock(), createBlock(unref(SelectScrollDownButton_default), {
									key: 1,
									class: normalizeClass([unref($style).selectScrollButton, unref($style).selectScrollButtonDown]),
...
var Select_vue_vue_type_style_index_0_lang_module_default = {
	selectTrigger: "_selectTrigger_1wvu1_115",
	clearButton: "_clearButton_1wvu1_148",
	variantGhost: "_variantGhost_1wvu1_170",
	variantFlush: "_variantFlush_1wvu1_182",
	mini: "_mini_1wvu1_204",
	small: "_small_1wvu1_211",
	medium: "_medium_1wvu1_218",
	large: "_large_1wvu1_225",
	xlarge: "_xlarge_1wvu1_232",
	selectedIcon: "_selectedIcon_1wvu1_239",
	selectedIconGlyph: "_selectedIconGlyph_1wvu1_247",
	trailingIcon: "_trailingIcon_1wvu1_275",
	selectContent: "_selectContent_1wvu1_280",
	viewportRegion: "_viewportRegion_1wvu1_296",
	selectViewport: "_selectViewport_1wvu1_304",
	empty: "_empty_1wvu1_310",
	footer: "_footer_1wvu1_318",
	selectValue: "_selectValue_1wvu1_330",
	selectItem: "_selectItem_1wvu1_339",
	selectLabel: "_selectLabel_1wvu1_363",
	selectSeparator: "_selectSeparator_1wvu1_369",
	selectScrollButton: "_selectScrollButton_1wvu1_376",
	selectScrollButtonUp: "_selectScrollButtonUp_1wvu1_399",
	selectScrollButtonDown: "_selectScrollButtonDown_1wvu1_406"
};
