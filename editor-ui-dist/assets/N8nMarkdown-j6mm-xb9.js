...
import { t as require_markdown_it } from "./markdown-it-Capwas2j.js";
	...
	setup(__props, { emit: __emit }) {
		...
		const htmlContent = computed(() => {
			...
			if (props.withMultiBreaks) {
				...
				const renderExtraBlankLines = (text) => text.replace(/\n{3,}/g, (match) => {
				...
				for (const line of contentToRender.split("\n")) {
					...
					if (previous && previous.isCode === isCode) previous.lines.push(line);
					else segments.push({
						isCode,
						lines: [line]
					});
				}
				contentToRender = segments.map(({ isCode, lines }) => isCode ? lines.join("\n") : renderExtraBlankLines(lines.join("\n"))).join("\n");
			}
		...
		return (_ctx, _cache) => {
			return openBlock(), createElementBlock("div", { class: normalizeClass({ "n8n-markdown": applyGlobalMarkdownStyles.value }) }, [!__props.loading ? (openBlock(), createElementBlock("div", {
				key: 0,
				ref_key: "editor",
				ref: editor,
...
var Markdown_vue_vue_type_style_index_0_lang_module_default = {
	markdown: "_markdown_6dfjq_1",
	label: "_label_6dfjq_43",
	sticky: "_sticky_6dfjq_64",
	spacer: "_spacer_6dfjq_218"
};
//#endregion
