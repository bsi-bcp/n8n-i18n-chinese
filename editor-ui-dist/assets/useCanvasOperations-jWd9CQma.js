...
import { s as useI18n$1 } from "./src-Bo6fIRlP.js";
import { t as _plugin_vue_export_helper_default } from "./_plugin-vue_export-helper-D-F0WtqU.js";
import { d as useClipboard, x as useLocalStorage } from "./dist-CaaDNBsJ.js";
import { t as require_markdown_it_link_attributes } from "./markdown-it-link-attributes-DZgP-5Xw.js";
...
import { Br as doRectsOverlap, Dr as mapLegacyConnectionToCanvasConnection, Er as mapCanvasConnectionToLegacyConnection, Et as useCredentialsStore, Fr as DEFAULT_NODE_SIZE, Gn as getAllNodeCredentialForAuthType, H as useNodeTypesStore, Hi as isValidNodeConnectionType, Ir as DEFAULT_VIEWPORT_BOUNDARIES, It as useDataSchema, Jr as getNodesGroupSize, Kr as getNewNodePosition, Lr as HORIZONTAL_NODE_STEP, Nr as CONFIGURABLE_NODE_SIZE, O as injectWorkflowDocumentStore, Or as mapLegacyConnectionsToCanvasConnections, Pr as CONFIGURATION_NODE_SIZE, Rr as PUSH_NODES_OFFSET, T as createWorkflowDocumentId, V as require_uniq, Vr as generateOffsets, Xn as getMainAuthField, Xr as snapToGrid, Yi as CanvasConnectionMode, Zi as CanvasNodeRenderType, _ as disposeWorkflowExecutionStateStore, an as ensureNodePosition, bn as useProjectsStore, br as createCanvasConnectionHandleString, bt as getN8nAgentsNodeName, cr as isRequiredCredential, en as useWorkflowHelpers, fn as useNDVStore, in as useTagsStore, j as useWorkflowDocumentStore, kr as parseCanvasConnectionHandleString, ot as removePreviewToken, sn as sanitizeConnections, t as useWorkflowsStore, u as getExecutionErrorToastConfiguration, vi as pinDataToExecutionData, y as useWorkflowExecutionStateStore, z as serializeNode } from "./workflows.store-CyNGMYqF.js";
import { t as core_default } from "./core-CSGkYda6.js";
import { a as python, i as typescript, n as javascript, r as bash, t as xml } from "./xml-Dhz3r8WZ.js";
import { t as useRootStore, ur as defineStore } from "./useRootStore-zV3ddzsk.js";
import { Ca as isObjectEmpty, Ei as generateNodesGraph, Mi as getNodeInputs, Ni as getNodeOutputs, Pi as getNodeParameters, Qi as resolveNodeWebhookId, Qn as validateNodeSelectionForGrouping, Sa as isCommunityPackageName, Ui as isHitlToolType, Wn as require_cloneDeep, Zn as validateNodeSelectionForExtraction, ba as deepCopy, ji as getConnectionTypes, lo as v4, pa as NodeConnectionTypes, ua as isResourceLocatorValue, zi as getSubworkflowId } from "./src-BvYowTlb.js";
...
import { $a as NDV_IN_FOCUS_PANEL_EXPERIMENT, Da as CANVAS_ZOOMED_VIEW_EXPERIMENT, Ft as CHAT_TRIGGER_NODE_TYPE, Gi as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL_SUB_NODE, Ki as LOCAL_STORAGE_LOGS_PANEL_OPEN, Wi as LOCAL_STORAGE_LOGS_PANEL_DETAILS_PANEL, Wn as STICKY_NODE_TYPE, Yn as UPDATE_WEBHOOK_ID_NODE_TYPES, fn as MANUAL_CHAT_TRIGGER_NODE_TYPE, fo as EnterpriseEditionFeature, li as MODAL_CONFIRM, ln as KEEP_AUTH_IN_NDV_FOR_NODES, qi as LOCAL_STORAGE_LOGS_SYNC_SELECTION } from "./constants-CfolRcla.js";
import { a as TELEMETRY_EVENT } from "./src-7WmJaBUb.js";
import { t as usePostHog } from "./posthog.store-BzH8pehx.js";
import { _ as tryToParseNumber, c as isEmpty, d as isPresent } from "./typesUtils-MNq3j-_F.js";
import { o as getNewWorkflowData } from "./workflowsList.store-btNOTGAu.js";
import { n as useUIStore } from "./ui.store-DF3DguxG.js";
import { _ as useCanvasStore, a as AddNodeGroupCommand, d as RemoveNodeCommand, f as RemoveNodeGroupCommand, h as UpdateNodeGroupCommand, i as AddNodeCommand, l as MoveNodeCommand, m as ReplaceNodeParametersCommand, n as useHistoryStore, p as RenameNodeCommand, r as AddConnectionCommand, t as useNodeHelpers, u as RemoveConnectionCommand } from "./useNodeHelpers-D1AK6w9s.js";
...
import { t as clearAllNodeResourceLocatorValues } from "./templateTransforms-B3Cc4U4g.js";
...
function useNodeCredentialOptions(node, nodeType, overrideCredType, displayAllOptions = false, overrideCredentials = void 0) {
	const nodeHelpers = useNodeHelpers();
	...
	function getCredentialOptions(types) {
		...
		let options = [];
		types.forEach((type) => {
			const source = override ? override.filter((credential) => credential.type === type) : credentialsStore.allUsableCredentialsByType[type];
			options = options.concat(source?.map((option) => ({
				...option,
...
*/
function getAutoSelectedCredential(node, overrideCredType = "", overrideCredentials) {
	if (!isEmpty(node.credentials ?? {})) return void 0;
	const { credentialTypesNodeDescriptionDisplayed } = useNodeCredentialOptions(node, useNodeTypesStore().getNodeType(node.type, node.typeVersion), overrideCredType, false, overrideCredentials);
...
*/
function getAgentNodeHandleOffset(height) {
	return snapToGrid(height / 2);
}
/** CSS `top` of the card's main handle `index` of `count`: getAgentNodeHandleOffset in CSS. */
function getAgentNodeHandleOffsetCss(index, count) {
	return `round(round(100%, 1px) * ${index + 1} / ${count + 1}, 16px)`;
}
//#endregion
...
var MessageWithButtons_vue_vue_type_style_index_0_lang_module_default = { buttons: "_buttons_omuwb_1" };
...
var useAgentNodeCanvasGeometryStore = defineStore("agentNodeCanvasGeometry", () => {
	const measurementsByCanvas = shallowReactive(/* @__PURE__ */ new Map());
	const contentKeysByCanvas = /* @__PURE__ */ new Map();
	const pendingCenterYByCanvas = /* @__PURE__ */ new Map();
	function setNodeMeasurement(canvasId, nodeId, measurement) {
		let measurementsByNode = measurementsByCanvas.get(canvasId);
		if (!measurementsByNode) {
			measurementsByNode = shallowReactive(/* @__PURE__ */ new Map());
			measurementsByCanvas.set(canvasId, measurementsByNode);
		}
		measurementsByNode.set(nodeId, measurement);
	}
	function getNodeMeasurement(canvasId, nodeId) {
...
function useCanvasOperations() {
	...
	* Node operations
	*/
	function tidyUp({ result, source, target, targetNodeCount }, { trackEvents = true, trackHistory = true, trackBulk = true } = {}) {
		updateNodesLayout(result.nodes, {
			trackBulk,
			trackHistory
		});
	...
	function trackTidyUp({ result, source, target, targetNodeCount }) {
	...
	function getStickyParametersForLayout(node, layoutNode) {
		if (node.type !== "n8n-nodes-base.stickyNote") return void 0;
		if (layoutNode.width === void 0 || layoutNode.height === void 0) return void 0;
		if (node.parameters.width === layoutNode.width && node.parameters.height === layoutNode.height) return;
		return {
			...node.parameters,
	...
	function updateNodesLayout(layoutNodes, { trackHistory = false, trackBulk = true } = {}) {
		const updates = layoutNodes.flatMap((layoutNode) => {
			const node = workflowDocumentStore.value.getNodeById(layoutNode.id);
			if (!node) return [];
			const positionChanged = node.position[0] !== layoutNode.x || node.position[1] !== layoutNode.y;
		...
		for (const { layoutNode, node, parameters, positionChanged } of updates) {
			if (positionChanged) updateNodePosition(layoutNode.id, {
				x: layoutNode.x,
				y: layoutNode.y
			}, { trackHistory });
			if (parameters) replaceNodeParameters(layoutNode.id, node.parameters, parameters, {
				trackHistory,
				trackBulk: false
			});
		}
	...
	function updateNodePosition(id, position, { trackHistory = false } = {}) {
		...
		if (oldPosition[0] === newPosition[0] && oldPosition[1] === newPosition[1]) return;
		...
		if (lastInteractedWithNode.value) {
			...
			if (newNodeInsertPosition) {
				...
				if (outputTypes.length > 0 && outputTypes.every((outputName) => outputName !== NodeConnectionTypes.Main)) {
					...
					const sourceHandleY = isAgentNodeV2(lastInteractedWithNodeObject) ? getAgentNodeHandleOffset(agentNodeCanvasGeometryStore.getNodeHeight(workflowDocumentStore.value.workflowId, lastInteractedWithNodeObject.id) ?? AGENT_NODE_SIZE[1]) : DEFAULT_NODE_SIZE[1] / 2;
					const targetHandleY = isAgentNodeV2(node) ? getAgentNodeHandleOffset(AGENT_NODE_SIZE[1]) : nodeSize[1] / 2;
					const centeredY = lastInteractedWithNode.value.position[1] + sourceHandleY - targetHandleY;
...
//#endregion
export { useI18n as A, useAiSimulatedDataGuard as B, ChatOptionsSymbol as C, MessageWithButtons_default as D, MessageComponentKey as E, getAgentNodeHandleOffsetCss as F, isAgentNodeV2 as I, getAutoSelectedCredential as L, useChat as M, AGENT_NODE_SIZE as N, MarkdownRenderer_default as O, getAgentNodeHandleOffset as P, useNodeCredentialOptions as R, shouldBlockUserInput as S, localStorageSessionIdKey as T, useAiSimulatedExecutionsStore as V, LOGS_PANEL_STATE as _, snapshotGroup as a, constructChatWebsocketUrl as b, addTokenUsageData as c, isChatNode as d, parseAiContent as f, LOCAL_STORAGE_PANEL_WIDTH as g, LOCAL_STORAGE_PANEL_HEIGHT as h, deleteGroupWithHistory as i, useOptions as j, Button_default as k, emptyTokenUsageData as l, LOCAL_STORAGE_OVERVIEW_PANEL_WIDTH as m, useAgentNodeCanvasGeometryStore as n, useSelectionValidation as o, useLogsStore as p, useCanvasNodeGroupTelemetry as r, useExperimentalNdvStore as s, useCanvasOperations as t, formatTokenUsageCount as u, LOG_DETAILS_PANEL_STATE as v, ChatSymbol as w, parseBotChatMessageContent as x, chatEventBus as y, useWorkflowNormalization as z };
