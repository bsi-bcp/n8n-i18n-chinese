...
| 结论 | 条件 | 动作 |
|---|---|---|
| 自动发版 | 补丁非红 + `AUTO_RELEASE=true`（仓库变量）+ 非演练 | `gh workflow run node.js.yml -f version=<ver>`，并轮询结果最长 45 分钟 |
| 等待确认 | 绿/黄灯但开关未开 | 飞书卡片附一键发版命令 |
| 需人工 | 红灯 | 自动建 `n8n-compat` Issue（同版本查重）+ 飞书红卡，人工适配补丁后 `force_version` 重评 |

> 基础镜像状态在评估卡中**仅作信息报告**（2026-09-18 补丁包模式起不再阻塞发版——完整包人工构建前人工确认即可）。
> ⚠️ 现行逻辑**黄灯（模糊贴合）也会自动发版**，卡片会提示人工复核补丁内容；如需黄灯也停，改 watcher 的 Auto release 步骤条件即可。

### 2.5 阶段五：补丁包流水线（node.js.yml，2026-09-18 收窄）
...
2. **词典覆盖率门禁**：统计 zh 缺失键，>0 自动补翻一轮（瞬时故障自愈），补后覆盖率 <99% 阻断发版（堵住 LLM API 耗尽时静默带病入库的缺口）；
3. **组装补丁包**：`n8n-zh-patch-<ver>.tar.gz` = zh-CN 词典 + 4 个源码补丁 + `script/en.json` 基准（+ en-nodes.json）+ `patch-manifest.json`（版本/键数/补丁清单）+ `README-应用说明.md`；
4. **打 tag 前 `git rebase origin/main`**：根除「构建窗口内 main 的 workflow 提交进入 tag 历史」被 GitHub 拒收的竞态（GITHUB_TOKEN 无 workflows scope 可授，2.39.8 实锤）；
5. **发布**：打 `release/x.y.z` tag → GitHub Release，**附件为补丁包**（不含完整 dist）。

> 🔴 **完整 n8n 汉化包（editor-ui dist / Docker 镜像）不在自动化范围内**——按 runbook 人工构建（用户 2026-09-18 决策；CI 全量构建在 2.39.8 首日实战连环暴露 3 缺陷：dispatch 403 假绿、tag 推送被拒、workflows scope 非法）。image.yml 保留为人工重推镜像的工具。

### 2.6 阶段六：镜像（人工 runbook）

- 自动化不再产出镜像；完整包按 CLAUDE.md runbook 在 TX-43 本地构建推 `registry.bcpcloud.cn/bcp/bcphub-n8n/n8n-chinese:<ver>`（FROM `bcp/n8n:<ver>` + COPY dist + `N8N_DEFAULT_LOCALE=zh-CN`）；
- `image.yml` 保留人工 dispatch 能力（版本 semver 白名单 → buildx multi-arch 推 SWR → 成功绿卡/失败红卡），仅作重推/补推工具，入口已无人触发。
...
| 场景 | 动作 |
|---|---|
| **完整包构建（常规，2026-09-18 起自动化不再产出）** | 按 CLAUDE.md runbook：本地翻译对齐 → n8n 源码注入+三补丁 → 过滤安装+构建 dist → TX-43 构建推 SWR 镜像 → YTJ1 灰度四件套 |
...
| 2026-09-18 续 | `workflows:write` 被证伪：GITHUB_TOKEN 不支持该 scope，声明即整文件 startup_failure（0s 失败运行），并连锁使 dispatch 403——watcher 的「dispatch-failed 红卡 + 不入状态」机制首次实战即正确兜底。**用户决策收窄自动化：node.js.yml 重写为补丁包模式**（翻译 + 覆盖率门禁 + 补丁包 tar 附件 + Release；删除 CI 全量构建与 image.yml 联动），打 tag 前 rebase origin/main 根治竞态；基础镜像移出发版门禁（仅信息报告）；完整 dist/镜像回归人工 runbook。image.yml 保留为人工重推工具 |
...
| 项 | 判定 | 是否阻塞 |
|---|---|---|
| 补丁贴合 | `git apply --check` 逐个试贴；✅→绿，🟡fuzz≤3 可过→黄，❌全失败→**只有 `feat__i18n_zhCn.patch` 判红** | **红阻断**；其余 patch（如 fix_editor-ui，2.38+ 已不需要）失败仅报告 |
| 翻译缺口 | 与 translate.js 同口径三方对比 | **不阻塞**（node.js.yml 内置增量翻译自动补） |
| 基础镜像 | DockerHub `n8nio/n8n:<ver>` 存在性 | **不阻塞**（仅信息报告；完整包人工构建前人工确认） |

**自动发版 5 条件**（全满足才 dispatch node.js.yml）：未早退 ∧ 补丁非红 ∧ `AUTO_RELEASE=true` ∧ 非 report_only ∧ 非 drill。红灯 → 建 `n8n-compat` Issue（同版本查重），人工适配后 `force_version` 重评。
